...
# 1. Initialize the kill switch in session state
        ...
        with col1:
            if st.button("🚀 Score Pending Files", use_container_width=True):
                st.session_state.stop_scoring = False  # Reset the switch
                
                with st.spinner("Scoring in progress...Also live to look new entries to score."):
                    status_area = st.empty()
                    # Pass the stop check into your function
                    # Note: You'll need to modify scoring() to accept this check
                    results=scoring()
                    n=len(results)
                    st.success(f"✅ Audit Complete: {n} files processed.") 

        with col2:
            if st.button("🛑 Terminate Scoring", type="primary", use_container_width=True):
                st.session_state.stop_scoring = True
                st.warning("Termination signal sent. Stopping after current file...")
            ...
            with st.status("Auditing Folder...", expanded=True) as status:
                ...
                if not local_files:
                    ...
                    try:
                        ...
                        if skipped_files:
                            with st.expander(f"ℹ️ {len(skipped_files)} files already in database"):
                        ...
                        if not files_to_process:
                            ...
                            for file_path in files_to_process:
                                ...
                                with file_container:
                                    ...
                                    with col1:
                                        st.markdown(f"🎙️ **Transcribing:** `{file_path.name}`")
                                        # Ensure this is NOT running in a separate thread if it needs to return data
                                        transcribe_single_file(str(file_path)) 
                                        st.caption("✨ Transcription saved to DB")
    
                                    st.divider() # Visual break between files
